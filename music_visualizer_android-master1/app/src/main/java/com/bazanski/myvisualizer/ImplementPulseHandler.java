...
public class ImplementPulseHandler implements PulseHandlerInterface {
    ...
    public Boolean ConnectMasterDevice(Activity activity) {
        ...
        } else if(this.bConnectMasterDevice.booleanValue()) {
            return Boolean.valueOf(true);
        } else {
            BluetoothHelper.getA2DPProfileProxy(activity, new ServiceListener() {
                public void onServiceConnected(int profile, BluetoothProfile proxy) {
                    List deviceList = proxy.getConnectedDevices();
                    if(deviceList.size() == 0) {
                        Log.i("hello", "deviceList.size()==0");
                    } else if(deviceList.size() == 1) {
                        ImplementPulseHandler.this.bluetoothDevice = (BluetoothDevice)deviceList.get(0);
                        ImplementPulseHandler.this.mSocket = BluetoothHelper.connect(ImplementPulseHandler.this.bluetoothDevice);
                        if(ImplementPulseHandler.this.mSocket != null && ImplementPulseHandler.this.mSocket.isConnected()) {
                            try {
                                ImplementPulseHandler.this.is = ImplementPulseHandler.this.mSocket.getInputStream();
                                ImplementPulseHandler.this.os = ImplementPulseHandler.this.mSocket.getOutputStream();
                            } catch (Exception var5) {
                                var5.printStackTrace();
                            }

                            (ImplementPulseHandler.this.new MyMessageThread(null)).start();
                            SppCmdHelper.init(ImplementPulseHandler.this.os);
                            SppCmdHelper.reqDeviceSoftwareVersion();
                            ImplementPulseHandler.this.bConnectMasterDevice = Boolean.valueOf(true);
                            ImplementPulseHandler.this.pulseNotifiedListener.onConnectMasterDevice();
                        }
                    } else if(deviceList.size() > 1) {
                ...
                public void onServiceDisconnected(int profile) {
                    ...
                    ImplementPulseHandler.this.bConnectMasterDevice = Boolean.valueOf(false);
                    ImplementPulseHandler.this.pulseNotifiedListener.onDisconnectMasterDevice();
                }
            });
            return Boolean.valueOf(true);
        }
    ...
    private class MyMessageThread extends Thread {
        ...
        public void run() {
            ...
            while(!Thread.interrupted()) {
                if(ImplementPulseHandler.this.is == null) {
                    return;
                }
                ...
                try {
                    int e = ImplementPulseHandler.this.is.read(msg);
                    ...
                    ImplementPulseHandler.this.processMessage(buffer);
